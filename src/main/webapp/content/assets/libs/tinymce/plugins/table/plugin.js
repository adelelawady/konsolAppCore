...
(function () {
  ...
  const getData$1 = (editor, cells) => {
    const cellsData = table(cells[0]).map(table =>
      map(getSelectedCells(table, cells), item => extractDataFromCellElement(editor, item.element, hasAdvancedCellTab(editor), item.column))
    );
  ...
  const addMenuItems = (editor, selectionTargets) => {
    ...
    if (tableClassList.length !== 0 && editor.queryCommandSupported('mceTableToggleClass')) {
      editor.ui.registry.addNestedMenuItem('tableclass', {
        ...
        text: 'Table styles',
        getSubmenuItems: () =>
          buildMenuItems(editor, tableClassList, 'tableclass', value => editor.execCommand('mceTableToggleClass', false, value)),
        onSetup: selectionTargets.onSetupTable,
      });
    }
    const tableCellClassList = filterNoneItem(getCellClassList(editor));
    ...
    if (editor.queryCommandSupported('mceTableApplyCellStyle')) {
      ...
      editor.ui.registry.addNestedMenuItem('tablecellborderwidth', {
        ...
        text: 'Border width',
        getSubmenuItems: () =>
          buildMenuItems(editor, getTableBorderWidths(editor), 'tablecellborderwidth', applyTableCellStyle(editor, 'border-width')),
        onSetup: selectionTargets.onSetupCellOrRow,
      });
      editor.ui.registry.addNestedMenuItem('tablecellborderstyle', {
        ...
        text: 'Border style',
        getSubmenuItems: () =>
          buildMenuItems(editor, getTableBorderStyles(editor), 'tablecellborderstyle', applyTableCellStyle(editor, 'border-style')),
        onSetup: selectionTargets.onSetupCellOrRow,
      });
