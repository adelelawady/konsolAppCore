(function (global, factory) {
  ...
  function setupZoomButtons() {
    ...
    var handler = function handler(zoomin) {
      ...
      return function () {
        return _this._setScale(
          zoomin ? _this.scale * _this.params.zoomStep : _this.scale / _this.params.zoomStep,
          _this._width / 2,
          _this._height / 2,
          false,
          _this.params.zoomAnimate
        );
      };
    };
  ...
  function createMarkers(markers, isRecentlyCreated) {
    ...
    var _loop = function _loop() {
      ...
      if (isRecentlyCreated) {
        if (
          Object.keys(_this._markers).filter(function (i) {
        ...
        switch (this._series.config.attribute) {
          ...
          case 'image':
            sample.style.background =
              'url(' + (typeof ticks[i].value === 'object' ? ticks[i].value.url : ticks[i].value) + ') no-repeat center center';
  ...
  var Proj = {
    ...
    lcc: function lcc(lat, lng, c) {
      var fi0 = 0,
        ...
        n =
          Math.log(Math.cos(fi1) * (1 / Math.cos(fi2))) / Math.log(Math.tan(Math.PI / 4 + fi2 / 2) * (1 / Math.tan(Math.PI / 4 + fi1 / 2))),
